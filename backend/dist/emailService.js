...
exports.resolveStaffEmail = resolveStaffEmail;
exports.sendSignInEmail = sendSignInEmail;
exports.sendSignOutEmail = sendSignOutEmail;
exports.sendStillOnSiteEmail = sendStillOnSiteEmail;
exports.sendTrainingCompletionEmail = sendTrainingCompletionEmail;
exports.testEmailConnection = testEmailConnection;
const certificate_1 = require("./certificate");
const graphMailer_1 = require("./graphMailer");
...
function resolveStaffEmail(meetingWith) {
...
function renderEmailHtml(headerColor, title, intro, rows, alertHtml) {
    const rowsHtml = rows
        .map(([label, value]) => `
                <tr>
...
async function safeSend(opts, context) {
    try {
        ...
        await (0, graphMailer_1.sendGraphMail)(opts);
...
async function sendSignInEmail(v) {
    ...
    await safeSend({
        ...
        html: renderEmailHtml('#0078d4', 'Visitor Sign-In Notification', 'A visitor has signed in to meet with you', [
...
async function sendSignOutEmail(v) {
    const recipient = resolveStaffEmail(v.meetingWith);
    if (!recipient) {
    ...
    await safeSend({
        ...
        html: renderEmailHtml('#d83b3b', 'Visitor Sign-Out Notification', 'Your visitor has signed out', [
            ['Visitor Name', `${v.firstName} ${v.lastName}`],
            ['Plant', v.plant ?? ''],
            ['Sign-in Time', formatCentral(v.createdAt)],
...
async function sendStillOnSiteEmail(v) {
    const recipient = resolveStaffEmail(v.meetingWith);
    if (!recipient) {
    ...
    await safeSend({
        ...
        html: renderEmailHtml('#e8a317', 'Late Visitor Alert', 'Your visitor is still on-site past 5:00 PM Central', [
            ['Visitor Name', `${v.firstName} ${v.lastName}`],
            ['Plant', v.plant ?? ''],
            ['Email', v.email],
            ['Phone', v.phone],
...
async function sendTrainingCompletionEmail(v, cert) {
    const recipient = resolveStaffEmail(v.meetingWith);
    if (!recipient) {
    ...
    let attachments = [];
    try {
        const pdfBuffer = await (0, certificate_1.generateCertificatePdf)(cert);
        attachments = [
            {
    ...
    await safeSend({
        ...
        html: renderEmailHtml('#2e8540', 'Training Completion Notification', 'Your visitor has completed site-specific safety training', [
            ['Visitor Name', `${v.firstName} ${v.lastName}`],
...
async function testEmailConnection() {
    return (0, graphMailer_1.testGraphConnection)();
}
